...
export async function createQuestion(params) {
    ...
    let index = 0;
    let questionCount = await QuestionModel.countDocuments({difficulty: paramDifficulty});
    console.log(questionCount);
    if(questionCount == 0) {
        index = 1;
    } else {
        index = await QuestionModel
            .find({difficulty: paramDifficulty})
            .sort({index:-1}).limit(1);
        index = index[0].index + 1;
    }
    return new QuestionModel({index: index, difficulty: paramDifficulty, question: question});
}
